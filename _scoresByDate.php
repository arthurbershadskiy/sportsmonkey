...
?>
	  ...
	  $end = 10;
		...
		if ($db_found)  {			
				$query = "SELECT * FROM scores WHERE day='{$date}' ORDER BY GameID DESC";
...
$fquery = "SELECT * FROM scores WHERE day='{$date}'";
	...
	echo "<script> window.location.assign('scores.php'); </script>";
		...
		<div class="select"> <strong>Other Pages: </strong>
          <select onChange="window.location=this.value;">
			...
			$count = 1;
			$pages = 1; ?>
			<option value='scores.php?date=<?php echo $date ?>&pid=<?php echo $pages ?>' <?php echo ($page==$pages)?'selected':'' ?>><?php echo $pages ?></option>
			<?php 
